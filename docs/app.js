/* NISM X-A Study Hub — single-page study app
...
(function () {
  ...
  // ---------- Reading text size ----------
  var FKEY = "nismxa_readscale", readScale = 1;
  ...
  function renderLab() {
    ...
    document.title = "Financial Maths Lab — NISM X-A";
  }
  ...
  function renderHome() {
    var hero = '<div class="hero"><h1>🎯 NISM Series X-A Study Hub</h1>'
      + '<p>Investment Adviser (Level 1) — notes, flashcards & 2,000+ explained MCQs. Pick a chapter and study at your own pace.</p>'
    ...
    chapterNavEl.innerHTML = ""; highlightNav(null); document.title = "NISM X-A Study Hub";
  }
  ...
  function renderQuestions(num) {
    ...
    var qkey = "nismxa_quiz_" + num, saved = {};
  ...
  function renderPaper(num) {
    ...
    var qkey = "nismxa_paper_" + num, saved = {}, elapsed = 0, ekey = "nismxa_paper_time_" + num;
    try { saved = JSON.parse(localStorage.getItem(qkey)) || {}; } catch (e) {}
    ...
    document.title = parsed.title + " — NISM X-A";
    highlightNav(null);
  ...
  function renderPapersHome() {
    ...
    var hero = '<div class="hero" style="background:linear-gradient(135deg,#b45309,#dc2626)"><h1>📑 Mock Test Papers</h1>'
      + '<p>Ten <b>full-length 150-mark papers</b> built to the exact NISM X-A blueprint — 90 MCQs plus 9 caselets. Each is auto-scored with the real 25% negative marking and a 3-hour timer.</p></div>';
    var cards = '<div class="home-cards">';
    if (!PAPERS.length) cards += '<p class="markdown-body">Mock papers are being prepared — check back shortly.</p>';
    PAPERS.forEach(function (p) {
      var saved = {}; try { saved = JSON.parse(localStorage.getItem("nismxa_paper_" + p.num)) || {}; } catch (e) {}
      var att = Object.keys(saved).length;
      var pp = parsePaper(p.md);
      var nq = 0, marks = 0, ncase = 0, nmcq = 0;
      pp.groups.forEach(function (g) {
        nq += g.questions.length; marks += g.questions.length * g.marks;
        if (g.kind === "case") ncase++; else nmcq += g.questions.length;
      });
      cards += '<div class="home-card" data-go="#/paper/' + p.num + '">'
        + '<div class="hc-num">PAPER ' + p.num + "</div>"
        + '<div class="hc-title">' + marks + " marks · " + nq + " Qs</div>"
        + '<div class="hc-links"><span class="chip">📝 ' + nmcq + ' MCQs</span><span class="chip">🧩 ' + ncase + ' cases</span>'
        + (att ? '<span class="chip" style="background:var(--brand-soft);color:var(--brand)">' + att + "/" + nq + " done</span>" : "") + "</div></div>";
    });
    cards += "</div>";
    ...
    highlightNav(null);
    var pl = document.querySelector('.nav-tool[data-link="papers"]'); if (pl) pl.classList.add("active");
    document.title = "Mock Test Papers — NISM X-A";
  }
  ...
  function renderPrimer() {
    ...
    document.title = "Financial Maths Primer — NISM X-A";
  }
  ...
  function renderExcel() {
    ...
    document.title = "Excel TVM Calculator — NISM X-A";
  }
  ...
  function route() {
    ...
    if (m) {
      ...
      document.title = num + ". " + c.title + " — NISM X-A";
    } else renderHome();
  }
